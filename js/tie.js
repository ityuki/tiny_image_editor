const Tie = (function(){
  const $tie = this;
  const MODULE_NAME = "tie";

  const regExpEscape = this.regExpEscape = function regExpEscape(str) {
  ...
  */
  ...
  // storage class
  const Storage = this.Storage = class Storage {
    constructor(main) {
      this.main = main;
    }
    #realKeyPrefixName(){
      return MODULE_NAME + "_" + this.main.tieName + "_";
    }
    #realKeyName(key) {
        ...
        if (name !== '') {
          if (name.startsWith(gprefx)) {
            ...
            if (n.startsWith(prefix)){
              r.push(n.substring(prefx.length));
            }
          }
        } 
      }
  ...
  // element Util
  const ElementUtil = this.ElementUtil = class ElementUtil {
    constructor(main){
      this.main = main;
    }
    #getElementVisibleConfigPrefixName(){
      return 'conf.elem.visible.';
    }
    #getElementPositionConfigPrefixName(){
      return 'conf.elem.position.';
    }
    getRealId(name){
      return "tie_obj_" + this.main.tieName + "_" + name;
    }
    getRealClassNames(name){
      return ["tie_global_" + name,"tie_local_" + this.main.tieName + "_" + name];
    }
    createTag(tag, name, innerHTML, styles, options) {
      const obj = document.createElement(tag);
      if (typeof name === 'string') {
        obj.id = this.getRealId(name);
        obj.classList.add(... (this.getRealClassNames(name)));        
      }
    ...
    addText(object, text) {
      object.insertAdjacentHTML("beforeend",text);
    }
    setVisible(object,visible,save){
      object.style.display = visible !== false ? 'block' : 'none';
      if (save === true) {
        const objId = object.id;
        if (objId){
          this.main.storage.save(this.#getElementVisibleConfigPrefixName() + objId,visible);
        }   
      }
    }
    getSavedVisible(object, defaultVisible){
      const objId = object.id;
      if (objId){
        return this.main.storage.load(this.#getElementVisibleConfigPrefixName() + objId, defaultVisible);
      }
      return defaultVisible;
    }
    deleteSavedVisible(object) {
      const objId = object.id;
      if (objId){
        return this.main.storage.delete(this.#getElementVisibleConfigPrefixName() + objId);
      }
    }
    setPosition(object,type,value,save){
      object.style[type] = value + "px";
      if (save === true){
        const objId = object.id;
        if (objId){
          this.main.storage.save(this.#getElementPositionConfigPrefixName() + type + "." + objId);
        }  
      }
    }
    getSavedPosition(object,type,defaultValue){
      ...
      if (objId){
        return this.main.storage.load(this.#getElementPositionConfigPrefixName() + type + "." + objId,defVal);
      }
      return defVal;
    }
    deleteSavedPosition(object){
      const objId = object.id;
      if (objId){
        const prefix = this.#getElementPositionConfigPrefixName();
        const match = RegExp('^[A-Za-z0-9]+\\.' + regExpEscape(objId) + '$');
        for(let k of this.main.storage.keys(prefix)){
          const t = k.substring(prefix.length);
          if (match.test(t)){
            return this.main.storage.delete(k);
          }
        }
      }
    }
  ...
  // Main (export) class
  const Main = this.Main = class Main {
